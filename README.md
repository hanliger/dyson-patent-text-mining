...
└── outputs/                              # 노트북 실행 결과
    ...
    ├── wordclouds/                           # sub-cluster wordcloud PNG (README용)
    ├── llm_sub_cluster_summary_input.jsonl   # sub-cluster용 LLM payload
    ├── llm_sub_cluster_labels.jsonl          # sub-cluster label (LLM)
    ├── llm_sub_cluster_product_engineering.jsonl  # sub-cluster 모듈/요소 (LLM)
    ├── sub_cluster_modules.csv               # 평탄화
    ├── sub_cluster_functional_elements.csv
    ├── sub_cluster_physical_elements.csv
    ├── sub_cluster_function_physical_mappings.csv
    ├── sub_cluster_product_engineering_overview.csv
...
추가로, mixed cluster 10·11을 sub-cluster 5개(10.0, 10.1, 11.0, 11.1, 11.2)로 분리해 같은 LLM 추출을 다시 적용했고, sub-cluster 단위로
- 23 modules
- 32 functional elements
- 49 physical elements
- 37 function ↔ physical ↔ module mappings

이 추출됐다 (`outputs/sub_cluster_*.csv`). sub-domain별 추출은 mixed parent보다 훨씬 일관된 modules / elements 목록을 보여준다.
...
| 10.0 — Dental cleaning | 10.1 — Vacuum & floor tool |
|---|---|
| ![sub_cluster_10_0](outputs/wordclouds/sub_cluster_10_0.png) | ![sub_cluster_10_1](outputs/wordclouds/sub_cluster_10_1.png) |
...
| 11.0 — Hand dryer / drying | 11.1 — Air treatment / humidifier | 11.2 — Wearable purifier & filter |
|---|---|---|
| ![sub_cluster_11_0](outputs/wordclouds/sub_cluster_11_0.png) | ![sub_cluster_11_1](outputs/wordclouds/sub_cluster_11_1.png) | ![sub_cluster_11_2](outputs/wordclouds/sub_cluster_11_2.png) |

전체 sub-cluster summary는 `outputs/sub_cluster_summary.csv`, 문서→sub-cluster 매핑은 `outputs/patent_subclusters.csv`, sub-sweep eval 곡선은 `outputs/sub_clustering_eval.csv`, wordcloud PNG는 `outputs/wordclouds/` 참고.
...
**Sub-cluster LLM 추출** — 위 parent-level module/function/physical 목록은 dental과 vacuum이 섞인 mixed cluster 기준이므로, 아래 sub-cluster별 추출이 sub-domain 단위의 더 정확한 분석이다. 전체 CSV는 `outputs/sub_cluster_modules.csv` 등 참조.
...
**Sub-cluster LLM 추출** — 위 parent-level module/function/physical 목록은 hand dryer · humidifier · wearable purifier가 섞인 mixed cluster 기준이므로, 아래 sub-cluster별 추출이 sub-domain 단위의 더 정확한 분석이다. 전체 CSV는 `outputs/sub_cluster_modules.csv` 등 참조.
